...
#include "PacketManager/PSoCSerial.h"
...
Status FaceAuthenticatorImpl::Connect(const SerialConfig& config)
{
    try
    {
        ...
        _serial = std::make_unique<PacketManager::PSoCSerial>(serial_config);
        return Status::Ok;
    }
...
//        PacketManager::Timer session_timer {CommonValues::enroll_max_timeout};
        PacketManager::Timer session_timer {60000};
        while (true)
        {
...
Status FaceAuthenticatorImpl::Authenticate(AuthenticationCallback& callback)
{
    try
    {
        ...
        PacketManager::Timer session_timer {10000};
        while (true)
        {
            if (session_timer.ReachedTimeout())
            {
...
void FaceAuthenticatorImpl::AuthLoopSleep(uint32_t timeout)
{
    PacketManager::Timer timer {timeout};
    LOG_DEBUG(LOG_TAG, "AuthLoopSleep upto %lu millis", timeout);
    // sleep in small interval to stop sleep if we got canceled
    while (!timer.ReachedTimeout() && !_cancel_loop)
    {
        uint32_t sleep_interval = 500;
        ...
        Cy_SysLib_Delay(sleep_interval);
    }
}
...
Status FaceAuthenticatorImpl::AuthenticateLoop(AuthenticationCallback& callback)
{
    ...
    do
    {
        ...
        uint32_t sleep_for = clbk_handler.face_found() ? 600 : 2100;
